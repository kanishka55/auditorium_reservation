...
<?php 
 ...
 $s = "SELECT COUNT(*) AS num_events, DATE_FORMAT(date_application, '%Y-%m') AS month
 FROM booking_details
 GROUP BY DATE_FORMAT(date_application, '%Y-%m')";
...
<script>
  ...
  const labels = <?php echo json_encode($output['data1']) ?>;
  const data = {
    labels: labels,
    datasets: [{
      label: 'Events For Each Month',
      data: <?php echo json_encode($output['data']) ?>,
      backgroundColor: [
        'rgba(255, 99, 132, 0.2)',
        'rgba(255, 159, 64, 0.2)',
        'rgba(255, 205, 86, 0.2)',
